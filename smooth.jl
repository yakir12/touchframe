...
file = "example tracks/with_ball.csv"
xy = Point2f.(Tuple.(CSV.File(file)))
...
path = [Point2f(mean(p)[1:2]) for p in ps]
vel = [Point2f(mean(p)[3:4]) for p in ps]
...
p = @lift([Point2f(path[$(sl.value)])])
d = @lift([Point2f(vel[$(sl.value)])])
